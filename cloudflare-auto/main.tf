...
resource "cloudflare_argo_tunnel" "default" {
  for_each = {
    for index, tunnel in concat(["var.default_tunnel_name"], [for record in var.dns_records: record.zero_trust != null ? record.zero_trust.tunnel != null ? record.zero_trust.tunnel.name : "" : "" ]) : tunnel => tunnel
  }
