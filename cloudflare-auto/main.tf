...
resource "cloudflare_record" "dns" {
  for_each = {
    ...
    if record.zero_trust != null ? record.zero_trust["tunnel"] == null : true
  }
  zone_id         = data.cloudflare_zone.domain.zone_id
...
resource "cloudflare_access_application" "cf_app" {
  for_each = {
    ...
    if record.zero_trust != null && record.zero_trust["protected"] != null
  }
  zone_id          = data.cloudflare_zone.domain.zone_id
...
resource "cloudflare_argo_tunnel" "default" {
  for_each = {
    for index, tunnel in concat(["var.default_tunnel_name"], [for record in var.dns_records: record["zero_trust.tunnel.name"]]) : tunnel.name => tunnel
  }
