...
resource "cloudflare_argo_tunnel" "default" {
  for_each = {
    for index, tunnel in compact(concat(["var.default_tunnel_name"], [for record in var.dns_records: record.zero_trust != null ? record.zero_trust.tunnel != null ? record.zero_trust.tunnel.name : "" : "" ])) : index => tunnel
  }
  ...
  config {
    ...
    dynamic "ingress_rule" {
      ...
      content {
        ...
        service  = "${ingress_rule.value.zero_trust.tunnel.local-protocol}://${ingress_rule.value.zero_trust.tunnel.local-ip != null ? ingress_rule.value.zero_trust.tunnel.local-ip : ingress_rule.value.name}:${ingress_rule.value.zero_trust.tunnel.local-port}"
      }
    }
    ingress_rule {
